...
class ScriptArguments:
    ...
    seq_length: Optional[int] = field(default=256, metadata={"help": "the sequence length"})
    num_workers: Optional[int] = field(default=4, metadata={"help": "the number of workers"})

    time_id = time.strftime("%m.%d-%H:%M:%S")
    training_args: TrainingArguments = field(
        default_factory=lambda: TrainingArguments(
            output_dir="data/DPO/SFT" + f"/run_" + time.strftime("%m.%d-%H:%M:%S"),
            max_steps=9000,
            logging_steps=10,
            save_steps=100,
            eval_steps=100,
            ...
            run_name="GPT4All_DPO_SFT" + "_" + time.strftime("%m.%d-%H:%M:%S"),
            report_to="wandb",
        )
    )
...
def prepare_sample_text(example):
    ...
    text = f"Answer the following question in context:\n\nQuestion: {example['prompt']}\n\nAnswer: {example['chosen']}"
    return text
...
tokenizer = AutoTokenizer.from_pretrained("nomic-ai/gpt4all-j", trust_remote_code=True)
